...
import com.spring.cloud.productcatalog.pojo.LinkedProduct;
...
public class ProductControllerHateos extends BaseController {
	...
	public @ResponseBody List<LinkedProduct> searchProduct() {
		logger.info("Entered searchJob");
		List<Product> products=productService.searchProducts();
		List<LinkedProduct> linkedProducts=new ArrayList<>();
		products.forEach((each)->{
			LinkedProduct linkedProduct=new LinkedProduct(each);
			/*Link selfLink = linkTo(methodOn(ProductControllerHateos.class)
			          .searchProduct(linkedProduct.getProductID())).withSelfRel();*/
			//Link selfLink = new Link("http://localhost:8080/spring-security-rest/api/customers/10A");

			//linkedProduct.add(selfLink);
